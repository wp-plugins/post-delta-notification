...
?></form>
		...
		function pdn_ContentInsertion( $content ) {
			...
			if ( "https://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'] == get_permalink() ) {
				...
				$return .= '<div id="PostDeltaNotification_Unsub" style="' . ( $subscribed ? "" : "display: none;" ) . '"><p style="text-align: ' . $devOptions['footerAlignment'] . '"><span id="PostDeltaNotification_UnsubResultText"></span>You are following this posts updates via email.<br>
...
jQuery( "#PostDeltaNotification_UnsubButton" ).click( function(e){
		...
		myRequest.open( "POST", "' . site_url() . '/?postdeltanotification=unsubscribe&post=' . $my_post_id . '&email=' . urlencode( $my_email ) . '", false ); //the false makes it synchronous
...
jQuery( "#PostDeltaNotification_SubButton" ).click(function(e){
		...
		myRequest.open( "POST", "' . site_url() . '/?postdeltanotification=subscribe&post=' . $my_post_id . '&email=' . urlencode( $my_email ) . '", false ); //the false makes it synchronous
		myRequest.setRequestHeader( "Content-Type","text/xml; charset=utf-8" );
		myRequest.send();
		if ( myRequest.status == 200 ) {
			jQuery( "#PostDeltaNotification_Unsub" ).toggle();
		...
		function pdn_ParseRequest( $wp ) {
			if ( array_key_exists( 'postdeltanotification', $_GET ) ) {
				...
				$addresses = get_post_meta( $_GET['post'], 'PDN_subscribers', true );
				if ( is_array( $addresses ) ) {
					$key = array_search( $email, $addresses );
				} else {
					$key = false;
				}
				if ( $_GET['postdeltanotification'] == 'unsubscribe' ) {
					if ( $key !== false ) {
						unset( $addresses[$key] );
					} else {
						wp_die( "Not currently subscribed. " . count( $addresses ) . " subscribers.", "", array( "response" => 200 ) );
					}
				} else if ( $_GET['postdeltanotification'] == 'subscribe' ) {
					if ( $key !== false ) {
				...
				if ( count( $addresses ) == 0 ) {
					delete_post_meta( $_GET['post'], 'PDN_subscribers' );
					wp_die( "Success in PostDeltaNotification ParseRequest. " . count( $addresses ) . " subscribers.", "", array( "response" => 200 ) );
				} else {
					if ( update_post_meta( $_GET['post'], 'PDN_subscribers', $addresses ) === false ) {
...
if ( isset( $svvsd_PDN ) ) {
	...
	add_filter( 'the_content', array( &$svvsd_PDN, 'pdn_ContentInsertion' ), 10 );
